...
export class AppResponse {
  ...
  bytes(buf: Uint8Array | ArrayBuffer) {
    this._body = buf as BodyInit
    return this
  }
...
function paramDecoratorFactory(
  type: 'path_param' | 'param' | 'query' | 'body' | 'ctx',
  key?: string,
) {
  return function (target: any, propertyKey: string, index: number) {
    console.log(
      `Registering param: ${type} ${index} -> ${target.constructor.name}.${propertyKey} ${key}`,
    )
  }
}
...
export function Validate(schema: ZodType) {
  return function (target: any, propertyKey: string) {
    console.log(
      `Registering validation schema for ${schema} on ${target.constructor.name}.${propertyKey}`,
    )
  }
}
